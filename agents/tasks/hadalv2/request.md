...
## Verification architecture for this run

Build the game around a headless TypeScript simulation from the first work item.
Use the same simulation in the browser and in Node-based tests.
Use unit tests for individual rules and headless integration tests for gameplay scenarios.
Reserve browser tests for browser integration, presentation, audio, and performance.

When planning work items and their required evidence, apply sections 30 and 70.
Preserve the complete game scope, visual quality requirements, and spoiler rules.
This revision introduces no agent turn limits, token limits, or session deadlines.
...
- CSS for UI;
- native WebAudio;
- Vitest for required unit tests and headless gameplay integration tests in Node.
...
## Required simulation boundary

Keep gameplay state and rules independent of Three.js, DOM, WebGL, WebAudio, and browser storage.
Import the simulation and production world data in Node without browser globals or browser emulation.
Keep renderer construction and browser side effects outside those imports.

Expose a small API to create a simulation and advance it with input and a fixed timestep.
An API such as `createSimulation(world, seed)` and `step(state, input, dt)` is sufficient.
An equivalent class API is also acceptable.
Use this API for the browser game and every headless gameplay scenario.
Do not create a second simulation or simplified collision system for tests.

Keep movement, collision, oxygen, harvesting, crafting, gates, creature decisions, and encounter triggers in the simulation.
Represent player actions as data, including movement, interaction, and menu selections.
Convert browser events into those actions in a thin input adapter.
Render simulation state through Three.js and the UI.
Send simulation events to audio and persistence adapters.
Keep save serialization, parsing, and version handling independent of `localStorage`.
Use a browser adapter for actual storage reads and writes.

Advance simulation time through fixed steps without sleeps, animation frames, or real-time keyboard holds.
Use seeded randomness for reproducible scenarios.
Reset simulation state and the seed between scenarios.
When a browser frame contains multiple simulation steps, consume each one-shot action only once.
Keep visual interpolation separate from gameplay updates.

Keep this boundary small and concrete.
Do not build a generic engine, dependency-injection framework, or plugin system for testing.

## Reusable scenario harness

Build a reusable Node harness alongside the first playable gameplay loop.
Use production world geometry, resource placement, recipes, gates, and the actual spawn.
Advance the production simulation with normal player actions.
Expose helpers for fixed steps, input sequences, state assertions, and concise failure traces.
Include the seed, simulated time, position, input, and failed assertion in each failure trace.

Use short authored input sequences or waypoint routes for movement scenarios.
Waypoints select movement inputs. They must not assign player positions or bypass collision.
Give each scenario a finite simulated duration and an explicit success condition.
If the condition remains false, fail with the state trace.
These scenario bounds detect gameplay failures. They do not limit the coding agent's session.

Do not build an autonomous browser pilot to prove resource access or progression.
Use the headless scenario harness for those questions.
Keep the harness reusable across implementer and reviewer sessions.
...
This abstract check does not prove that physical routes are traversable.
Pair it with headless movement scenarios from section 70.
Verify required passages against production collision geometry and the capabilities available at that progression stage.
Do not infer physical access from chunk adjacency alone.
...
Establish the simulation boundary from section 30 before browser-dependent gameplay code spreads.
Deliver the reusable headless harness with the first movement and collision implementation.
Extend it with the complete resource-to-upgrade scenario as the base and crafting become available.
Provide separate documented commands for headless tests and browser tests.
Make `npm test` run the headless suite once and return its exit status.
Keep browser tests out of the default headless command.
...
Use the evidence layers in this section for every work item and the final handoff.
Assign each acceptance criterion to its relevant layer during planning.
Use headless tests as primary evidence for gameplay rules and progression.
Use browser evidence for browser behavior and presentation.
Do not require a complete browser playthrough for each work item.

## Unit tests in Node

Verify individual rules with direct inputs and state assertions:

- movement, collision boundaries, oxygen consumption, and refill rules
- resource interaction distance, cargo capacity, and resource depletion
- recipe costs, upgrade effects, and insufficient-material rejection
- death penalties, permanent upgrades, and respawn state
- save round trips, malformed saves, and version handling
- gate requirements, trigger conditions, and duplicate-trigger prevention
- creature senses, state transitions, and applicable simulation throttling

Use real gameplay functions in these tests.
Do not mock the rule under test or assert only that a method was called.

## Headless integration tests in Node

Run these scenarios through the production simulation and world data as the corresponding features become available:

1. Start at the actual spawn with a fresh game and starter equipment.
2. Swim to an actual first resource through normal movement and collision.
3. Interact to collect the materials for the first upgrade.
4. Return to the actual base and reach the required station.
5. Craft through the same gameplay action that the browser menu submits.
6. Verify resource costs and the resulting capability change.
7. Leave the base and demonstrate the improved capability through simulation.
8. Serialize the save and load it into a fresh simulation.
9. Verify that the upgrade and required progression state persist.

Keep steps 1 through 9 as one continuous core-loop scenario.
Add separate scenarios for death, depleted resources, insufficient materials, and relevant blocked routes.
Add physical route scenarios for required gates, shortcuts, and progression materials.
Pair those scenarios with `simulateCriticalPath()` to cover progression dependencies.
Verify ending triggers and save continuity headlessly as the ending becomes available.
Keep hidden scenario details and traces inside the repository under the existing spoiler rules.

Do not use teleportation, noclip, free materials, or direct state edits as evidence of route or progression reachability.
Use explicit fixtures only for isolated mechanics or presentation checks.
Label fixture-based evidence so it cannot imply fresh-save reachability.

If a gameplay scenario fails, inspect the simulation trace before changing the harness.
Distinguish a product defect from incorrect scenario inputs or expectations.
When correcting the harness, keep valid acceptance assertions intact.
Do not remove a failing assertion merely to obtain a passing result.

## Focused browser tests

Maintain one shared browser harness with documented install and run commands.
Use Playwright or an equivalent existing browser tool for this layer.
Reuse this harness across work items and reviewer sessions.
Keep browser actions short and directed at browser-specific behavior:

- fresh boot produces a visible WebGL scene without page or console errors
- keyboard input reaches the production simulation and visibly moves the player
- interaction prompts and crafting controls submit the intended gameplay actions
- the HUD reflects actual simulation state
- the storage adapter preserves state across an actual page reload
- resize and the required viewport produce usable layouts
- audio starts after user input
- credits and restart controls work

Use condition-based waits for UI transitions.
Use deterministic fixtures for focused UI checks that require a particular game state.
Do not substitute those fixtures for the headless fresh-save scenario.
Do not repeat long swimming routes in Chromium to prove simulation behavior.

## Visual, audio, and performance inspection

Inspect representative real browser scenes for composition, readable controls, movement feel, atmosphere, and audio.
Inspect major encounters for visual continuity and the performance targets from section 34.
Use private fixtures for late-game presentation without exposing spoilers in progress reports.
Record observed defects and the relevant scene or reproduction steps.
Headless passes do not establish visual quality, audio quality, or enjoyable play.
Retain final playtesting for pacing and the complete player experience.
Do not rebuild that playtest as a browser navigation script for every review.

## Implementation and review ownership

Deliver reusable test infrastructure as part of the implementation.
Keep product code, durable harness code, and their fixes within implementer ownership.
Reviewers must independently assess acceptance criteria and inspect test assumptions.
Reviewers can run the shared harness and add focused scenarios in their permitted scratch area.
Do not require reviewers to recreate the harness or install a separate browser suite for each work item.
If shared infrastructure needs a product change, report the concrete defect for the implementer.
If a probe repeatedly fails without a clear cause, request an independent diagnosis from an available subagent.
Give that subagent the failure trace, relevant code, review scope, and product read-only restriction.
Keep the parent responsible for the verdict and evidence.

Record commands, exit status, scenario names, and observed results in the role artifact.
Distinguish gameplay failures, harness failures, unavailable tooling, and presentation defects.
Do not report a missing browser observation as a headless pass.

## Final coverage checklist

Verify the following outcomes through their applicable evidence layers.
Use browser inspection for visible, audible, and usability claims.
Use headless evidence for rules, reachability, persistence logic, and trigger behavior.
